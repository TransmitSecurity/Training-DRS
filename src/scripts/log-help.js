...
const scriptList = {
  ...
  "build-test":
    "          Build the app and run the unit & e2e (headless) tests",
  ...
  "vite:inspect":
    "        Inspect the vite build on the localhost (after build)",
  "rollup:inspect":
    "      Inspect the rollup bundling on the localhost (after build)",
  coverage:
    "            Run the unit tests and generate a coverage report (c8)",
  "type-check": "          Type check the app",
  lint: "                Lint the app with ESlint",
  format: "              Format the app with prettier",
};
scripts.forEach((script) => {
  if (Object.keys(scriptList).includes(script))
    console.log(`${script}   ${scriptList[script]}`);
